...
export function getAssumptions() {
    const db = getDb();
    const row = db.prepare("SELECT * FROM assumptions WHERE id = 1").get();
    return {
        id: 1,
...
export function updateAssumptions(input) {
    const db = getDb();
    const stmt = db.prepare(`UPDATE assumptions SET
			down_payment_pct = ?,
			interest_rate_30yr = ?,
			interest_rate_15yr = ?,
			...
			vacancy_rate_pct = ?
		WHERE id = 1`);
    stmt.run(input.downPaymentPct, input.interestRate30yr, input.interestRate15yr, input.interestRate5yrArm, input.armAmortYears, input.propertyTaxRatePct, input.maintenanceRatePct, input.vacancyRatePct);
